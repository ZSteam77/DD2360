...
__global__ void gpu_kernel(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param){
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index > part-> nop) return;
    ...
    for (int i_sub=0; i_sub <  part->n_sub_cycles; i_sub++){
        // move each particle with new fields
            ...
            for(int innter=0; innter < part->NiterMover; innter++){
                ...
                // calculate weights
                xi[0]   = part->x[index] - grd->XN[ix - 1][iy][iz];
                eta[0]  = part->y[index] - grd->YN[ix][iy - 1][iz];
                zeta[0] = part->z[index] - grd->ZN[ix][iy][iz - 1];
                xi[1]   = grd->XN[ix][iy][iz] - part->x[index];
                eta[1]  = grd->YN[ix][iy][iz] - part->y[index];
                zeta[1] = grd->ZN[ix][iy][iz] - part->z[index];
                for (int ii = 0; ii < 2; ii++)
                    for (int jj = 0; jj < 2; jj++)
                        for (int kk = 0; kk < 2; kk++)
                            weight[ii][jj][kk] = xi[ii] * eta[jj] * zeta[kk] * grd->invVOL;
...
int mover_PC_gpu(struct particles* part, struct EMfield* field, struct grid* grd, struct parameters* param){
    ...
    particles* device_part;
    EMfield* device_field;
    grid* device_grid;
    parameters* device_param;

    cudaMalloc((void **)device_part, sizeof(particles));
    cudaMalloc((void **)device_field, sizeof(EMfield));
    cudaMalloc((void **)device_grid, sizeof(grid));
    cudaMalloc((void **)device_param, sizeof(parameters));
    ...
    cudaMemcpy(device_grid, grd, sizeof(grid), cudaMemcpyHostToDevice);
    cudaMemcpy(device_param, param, sizeof(parameters), cudaMemcpyHostToDevice);

    gpu_kernel<<<((part->nop / 32) + 1), 32>>>(device_part, device_field, device_grid, device_param);
    cudaDeviceSynchronize();

    cudaMemcpy(part, device_part, sizeof(particles), cudaMemcpyDeviceToHost);
    cudaMemcpy(field, device_field, sizeof(EMfield), cudaMemcpyDeviceToHost);
    cudaMemcpy(grd, device_grid, sizeof(grid), cudaMemcpyDeviceToHost);
    cudaMemcpy(param, device_param, sizeof(parameters), cudaMemcpyDeviceToHost);
